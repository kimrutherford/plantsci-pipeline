...
# <<Pipeline data>>
   ...
   =pipeline_directory= configuration options, in the sub-directory given by
   the =data_sub_directory= option.
** The pipeline server
# <<The pipeline server>>
   The script that controls the pipeline is =pipeserv.pl=.
   - tries to create new processes (entries in the =pipeprocess= table) using
